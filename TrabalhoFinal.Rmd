...
# Etapa 1 - Escolha da base de dados

##### Um dos desafios para a realização do trabalho, foi referente à busca de um dataset que fosse relacionado com a minha pesquisa de doutorado que consiste em análise de sentimento, neurociência e transtorno do espectro do autismo.Os datasets encontrados não continham uma descrição dos atributos, o que causou o não entendimento do dataset, outro desafio refere-se ao processamento do computador com baixo desempenho e dataset grandes, com isso, vários datasets interessantes tiveram que ser desconsiderados. Assim,o dataset escolhido no repositório Kaggle refere-se ao conjunto de dados que está disponivel em: https://www.kaggle.com/arslanali4343/top-personality-dataset

# Etapa 2 - Objeto de pesquisa

##### Este problema tenta identificar se indivíduos são receptivos para a diversidade, dado os indicadores de personalidade que constam no dataset. No artigo de Nguyen, Tien T. et al., 2018, os dados análisados são utilizados no contexto de sistemas de recomendação. 
...
```
#### 3.3.1 Exclusão de outliers do atributo openness
...
#### 3.3.2 Exclusão de outliers do atributo agreeableness
...
#### 3.3.3 Exclusão de outliers do atributo conscientiousness
```{r}
...
#### 3.3.4 Exclusão de outliers do atributo enjoy_watching
```{r}
...
#####  Optou-se por utilizar 2 técnicas diferentes, usando um algoritmo em cada. Dentre as técnicas vistas em aula, as escolhidas, devido aos dados do dataset, foram a agrupamento, utilizando o algoritmo kmeans e regressão, utilizando redes neurais.



## 4.1 Agrupamento -  k-means 

##### 4.1.2 Exclusão dos atributos categoricos.

```{r}
p <- subset(personality5, select = -c(assigned.metric, assigned.condition))
```

##### 4.1.3 Transformar em scala

```{r}
p <- scale(p)
```

##### 4.1.4 Função para encontrar a quantidade ótima de clusters

```{r}
fviz_nbclust(p, kmeans, method = "gap_stat")
```
##### 4.1.5 Aplicação do algoritmo kmeans com 5 clusters
```{r}
p_kmeans <- kmeans(p, 5)
```
...
##### 4.2.1 Transformação as classes serendipity, popularity, diversity, all para binário

##### 4.2.1.1 criação das colunas
```{r}
prede <- personality5
...
##### 4.2.1.2 Divisão o dataset 70% treino e 30% teste
...
##### 4.2.2.2 Matriz de confusão para melhorar entendimento dos resultados
...
##### 4.2.2.3 Grau de acerto da Rede Neural
...
##### É possível perceber a definição dos grupos no agrupamento, embora alguns pontos estejam distantes de seu cluster, como pode-se observar os pontos azuis e vermelhos, ainda, a maior parte deles estão próximos uns dos outros, dessa forma possuem caracteristicas parecidas, assim como os grupos verde e marrom  que são os que mais apresentam proximidade. Os grupos indicados nas cores azul, vermelho, lilás e marrom são semelhantes entre si. é notável a distância do grupo verde para o vermelho e lilás, indicando que eles são diferentes entre si. Isso identifica os indivíduos com características semelhantes.


##### Pode-se observar na matriz de confusão, que o modelo de rede neural, errou em bastante instância, talvez outra configuração de rede conseguiria obter resultados melhores.

##### Dessa forma, nesse trabalho, não foi possível identificar se indivíduos são receptivos para a diversidade, baseado em dados de questionário e filmes.
